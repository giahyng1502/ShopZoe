...
const LoginScreen = ({navigation, route}) => {
    ...
    useFocusEffect(
        useCallback(() => {
            notification();
        }, [notification]) // Truyền notification vào dependency
    );
    const getAccount = async () => {
        ...
        if (phoneNumber.length === 0 && passwordSave.length === 0) {
            ...
            const savePhoneNumber = await AsyncStorage('phoneNumber');
            const savePass = await AsyncStorage('password');
            setAccount(savePhoneNumber);
            setpassword(savePass);
            // console.log(password);
        }
    };
    useEffect(() => {
